  ...
  if (collapsible === 'none') {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          'flex h-svh w-(--sidebar-width) flex-col sticky top-0 bg-sidebar text-sidebar-foreground overflow-hidden',
          className,
        )}
        {...props}
      >
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      className={cn(
        'h-7 w-7 flex items-center justify-center rounded-[6px] hover:bg-sidebar-accent transition-colors',
        className,
      )}
      onClick={(event) => {
        onClick?.(event)
